...
for entry in df.index: # iterate through index file
    ...
    if pd.isna(entry) == False:
        ...
        weight = df.loc[entry, 'Girth (Inches)'] * df.loc[entry, 'Girth (Inches)'] * df.loc[entry, 'Length B (Sholder Bone to End of Thigh)'] / 661.0
        ...
        try :
            for root,d_names,f_names in os.walk(dest):
                ...
                for name in f_names:
                    ...
                    new_name=''
                    if side in name:
                        new_name = f'{entry}_s_{weight:.0f}_{age:.1f}_{sex[0]}.jpg'
                        # print(f" {entry} Side ref: {side} and image name is : {name} | newName : {new_name}  ")
                    elif rear in name:
                        new_name = f'{entry}_r_{weight:.0f}_{age:.1f}_{sex[0]}.jpg'
